...
% Log start
Event('Loading reference datasets');
tic;
...
Event('Initializing AP reference variables');
...
for i = 1:length(handles.MCapfiles)
    ...
    Event(['Loading DICOM info from reference file ', handles.MCapfiles{i}]);
    
    % Load width
    handles.MCap{i}.width = handles.MCap{i}.info.PixelSpacing;
    Event(sprintf('Pixel spacing %0.3f mm x %0.3f mm', handles.MCap{i}.width));
    
    % Load start
    handles.MCap{i}.start = [handles.MCap{i}.info.ImagePositionPatient(3); ...
        handles.MCap{i}.info.ImagePositionPatient(1)];
    Event(sprintf('Start position %0.3f mm, %0.3f mm', handles.MCap{i}.start));
    
    % Load data
    handles.MCap{i}.data = single(dicomread(handles.MCapfiles{i})) * ...
        handles.MCap{i}.info.DoseGridScaling;
    Event(sprintf('Image size %i x %i', size(handles.MCap{i}.data)));
    
    % Generate mesh
    [handles.MCap{i}.meshX, handles.MCap{i}.meshY]  = ...
        meshgrid(handles.MCap{i}.start(2):handles.MCap{i}.width(2):...
        handles.MCap{i}.start(2)+handles.MCap{i}.width(2) * ...
        (size(handles.MCap{i}.data,2)-1), ...
        handles.MCap{i}.start(1):handles.MCap{i}.width(1):...
        handles.MCap{i}.start(1)+handles.MCap{i}.width(1) * ...
        (size(handles.MCap{i}.data,1)-1));
    
    % Extract IEC-x axis data
    handles.MCap{i}.profile(1,:) = handles.MCap{i}.meshY(:,1);
    handles.MCap{i}.profile(2,:) = ...
        interp2(handles.MCap{i}.meshX, handles.MCap{i}.meshY, ...
        single(handles.MCap{i}.data), handles.MCap{i}.profile(1,:), ...
        zeros(1,size(handles.MCap{i}.profile,2)), '*linear');
    Event('IEC-x axis profile extracted using linear interpolation');
    ...
    for x = 1:I-1
        if handles.MCap{i}.profile(2,x) == C/2
            ...
            break;
        elseif handles.MCap{i}.profile(2,x) < C/2 && ...
                handles.MCap{i}.profile(2,x+1) > C/2
            handles.MCapX1(i) = interp1(handles.MCap{i}.profile(2,x:x+1), ...
                handles.MCap{i}.profile(1,x:x+1), C/2, 'linear');
            break;
        end
    end
    Event(sprintf('Profile X1 half max at %0.3f mm', handles.MCapX1(i)));
    ...
    Event(sprintf('Profile X2 half max at %0.3f mm', handles.MCapX2(i)));
    
    % Compute width
    handles.MCapFWHM(i) = handles.MCapX2(i) - handles.MCapX1(i);
    Event(sprintf('Profile FWHM computed as %0.3f mm', handles.MCapFWHM(i)));
end
...
Event('Initializing PA no couch reference variables');
...
for i = 1:length(handles.MCpancfiles)
    ...
    Event(['Loading DICOM info from reference file ', handles.MCpancfiles{i}]);
    
    % Load width
    handles.MCpanc{i}.width = handles.MCpanc{i}.info.PixelSpacing;
    Event(sprintf('Pixel spacing %0.3f mm x %0.3f mm', handles.MCpanc{i}.width));
    
    % Load start
    handles.MCpanc{i}.start = [handles.MCpanc{i}.info.ImagePositionPatient(3); ...
        handles.MCpanc{i}.info.ImagePositionPatient(1)];
    Event(sprintf('Start position %0.3f mm, %0.3f mm', handles.MCpanc{i}.start));
    
    % Load data
    handles.MCpanc{i}.data = single(dicomread(handles.MCpancfiles{i})) * ...
        handles.MCpanc{i}.info.DoseGridScaling;
    Event(sprintf('Image size %i x %i', size(handles.MCpanc{i}.data)));
    
    % Generate mesh
    [handles.MCpanc{i}.meshX, handles.MCpanc{i}.meshY]  = ...
        meshgrid(handles.MCpanc{i}.start(2):handles.MCpanc{i}.width(2):...
        handles.MCpanc{i}.start(2)+handles.MCpanc{i}.width(2) * ...
        (size(handles.MCpanc{i}.data,2)-1), ...
        handles.MCpanc{i}.start(1):handles.MCpanc{i}.width(1):...
        handles.MCpanc{i}.start(1)+handles.MCpanc{i}.width(1) * ...
        (size(handles.MCpanc{i}.data,1)-1));
    
    % Extract IEC-x axis data
    handles.MCpanc{i}.profile(1,:) = handles.MCpanc{i}.meshY(:,1);
    handles.MCpanc{i}.profile(2,:) = ...
        interp2(handles.MCpanc{i}.meshX, handles.MCpanc{i}.meshY, ...
    ...
    for x = 1:I-1
        if handles.MCpanc{i}.profile(2,x) == C/2
            ...
            break;
        elseif handles.MCpanc{i}.profile(2,x) < C/2 && ...
                handles.MCpanc{i}.profile(2,x+1) > C/2
            handles.MCpancX1(i) = interp1(handles.MCpanc{i}.profile(2,x:x+1), ...
                handles.MCpanc{i}.profile(1,x:x+1), C/2, 'linear');
            break;
        end
    end
    Event(sprintf('Profile X1 half max at %0.3f mm', handles.MCpancX1(i)));
    ...
    Event(sprintf('Profile X2 half max at %0.3f mm', handles.MCpancX2(i)));
    
    % Compute width
    handles.MCpancFWHM(i) = handles.MCpancX2(i) - handles.MCpancX1(i);
    Event(sprintf('Profile FWHM computed as %0.3f mm', handles.MCpancFWHM(i)));
end
...
Event('Initializing PA thru couch reference variables');
...
for i = 1:length(handles.MCpatcfiles)
    ...
    Event(['Loading DICOM info from reference file ', handles.MCpatcfiles{i}]);
    
    % Load width
    handles.MCpatc{i}.width = handles.MCpatc{i}.info.PixelSpacing;
    Event(sprintf('Pixel spacing %0.3f mm x %0.3f mm', handles.MCpatc{i}.width));
    
    % Load start
    handles.MCpatc{i}.start = [handles.MCpatc{i}.info.ImagePositionPatient(3); ...
        handles.MCpatc{i}.info.ImagePositionPatient(1)];
    Event(sprintf('Start position %0.3f mm, %0.3f mm', handles.MCpatc{i}.start));
    
    % Load data
    handles.MCpatc{i}.data = single(dicomread(handles.MCpatcfiles{i})) * ...
        handles.MCpatc{i}.info.DoseGridScaling;
    Event(sprintf('Image size %i x %i', size(handles.MCpatc{i}.data)));
    
    % Generate mesh
    [handles.MCpatc{i}.meshX, handles.MCpatc{i}.meshY]  = ...
        meshgrid(handles.MCpatc{i}.start(2):handles.MCpatc{i}.width(2):...
        handles.MCpatc{i}.start(2)+handles.MCpatc{i}.width(2) * ...
        (size(handles.MCpatc{i}.data,2)-1), ...
        handles.MCpatc{i}.start(1):handles.MCpatc{i}.width(1):...
        handles.MCpatc{i}.start(1)+handles.MCpatc{i}.width(1) * ...
        (size(handles.MCpatc{i}.data,1)-1));
    
    % Extract IEC-x axis data
    handles.MCpatc{i}.profile(1,:) = handles.MCpatc{i}.meshY(:,1);
    handles.MCpatc{i}.profile(2,:) = ...
        interp2(handles.MCpatc{i}.meshX, handles.MCpatc{i}.meshY, ...
    ...
    for x = 1:I-1
        if handles.MCpatc{i}.profile(2,x) == C/2
            ...
            break;
        elseif handles.MCpatc{i}.profile(2,x) < C/2 && ...
                handles.MCpatc{i}.profile(2,x+1) > C/2
            handles.MCpatcX1(i) = interp1(handles.MCpatc{i}.profile(2,x:x+1), ...
                handles.MCpatc{i}.profile(1,x:x+1), C/2, 'linear');
            break;
        end
    end
    Event(sprintf('Profile X1 half max at %0.3f mm', handles.MCpatcX1(i)));
    ...
    Event(sprintf('Profile X2 half max at %0.3f mm', handles.MCpatcX2(i)));
    
    % Compute width
    handles.MCpatcFWHM(i) = handles.MCpatcX2(i) - handles.MCpatcX1(i);
    Event(sprintf('Profile FWHM computed as %0.3f mm', handles.MCpatcFWHM(i)));
end

% Log completion
Event(sprintf('Reference datasets successfully loaded in %0.3f seconds', toc));
